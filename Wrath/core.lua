...
function func:CVars(event)
    if event == "VARIABLES_LOADED" then
        ...
        SetCVar("nameplateSelectedScale", 1.08);
    ...
    if unit then
        ...
        if unit == "player" then
            ...
            if nameplate then
                ...
                unitFrame.healthLeftSide:SetShown(Config.NumericValue and Config.Percentage);
    ...
    if unit then
        ...
        if nameplate then
            ...
            if text then
                ...
                if castbar.name:GetStringWidth() > 110 then
                    local spellName = castbar.name:GetText();

                    repeat
                        spellName = func:utf8sub(castbar.name:GetText(), 1, strlenutf8(spellName) - 1);
                        local spellNameTrimmed = strtrim(spellName);

                        castbar.name:SetText(spellNameTrimmed .. "...");
                    until( castbar.name:GetStringWidth() <= 110 )
                end
